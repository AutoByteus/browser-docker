...
- Desired outcome: Build the browser image from the official minimal Ubuntu 24.04 LTS OCI base while preserving the image's existing browser/VNC, runtime, locale-variant, and launch contracts.
- Observable definition of success: Built default and `zh` images report Ubuntu 24.04, build successfully for the supported architectures, and retain the documented services, tools, ports, user, and runtime behavior.
...
| BEH-002 | System | SCN-003 | A started image runs as `vncuser` and exposes the XFCE/TigerVNC/Chromium environment, remote debugging, websockify, persistent profile support, and optional Chinese input behavior. | Images built on Ubuntu 24.04 retain these product-visible and operational capabilities. | Existing user/UID/GID behavior, ports 5900/6080/9223, Chromium/VNC services, Python 3.11, Node.js 22, Yarn, `uv`, default English locale, `zh` option, and persistent profile behavior remain supported. | `Dockerfile`; `base.conf`; `entrypoint.sh`; `start-vnc.sh`; `docker-compose*.yml`; `run-container.sh`; `README.md`. |
...
- Broad package/runtime upgrades unrelated to compatibility with Ubuntu 24.04.
...
| REQ-004 | The upgraded image shall preserve the currently documented browser, desktop, VNC, websockify, remote-debugging, runtime/tooling, locale, input-method, user, port, and persistent-profile outcomes. | BEH-002 | Must | The request concerns the base OS age, not removal of existing capabilities. | Current repository sources and docs. |
...
| AC-006 | REQ-004, REQ-005 | BEH-002 / SCN-003 | Inspect runtime and tool contracts in the started default image. | `vncuser` and UID/GID customization work; Python 3.11, Node.js 22, Yarn, `uv`, and currently installed documented utilities are usable; `en_US.UTF-8` is generated. | Substitution with Ubuntu's default Python 3.12 or loss of named tooling without approval fails. | Shell version/identity/locale checks and representative command probes. |
...
| SCN-003 | System | Container runtime initiated by a downstream user/Compose | Run the existing browser/VNC environment on the upgraded image. | `run-container.sh` or repository Compose configuration. | An upgraded image is available; existing ports/profile volume are configured. | Runtime starts the container; supervisor starts services; user connects through existing VNC/debugging surfaces; optional `zh` behavior and profile lifecycle operate as documented. | Current product-visible and operational capabilities remain available on Ubuntu 24.04. | Existing startup/recovery logic handles supported stale VNC and Chromium lock conditions; other failures are observable in logs/exit state. | Supported Normal Scenario | `entrypoint.sh`; `start-vnc.sh`; `base.conf`; Compose/run scripts; README. | REQ-004–REQ-005; AC-005–AC-008. |
...
| Deadsnakes PPA | Python 3.11 packages required by the current image must resolve for Noble and supported architectures. | Deadsnakes Launchpad PPA publishes Python 3.11 for Noble, including AMD64/ARM64. | Third-party PPA availability remains an existing dependency risk. |
...
- Product and system constraints architecture must preserve: Explicit Ubuntu 24.04 LTS official minimal OCI base; default and `zh`; AMD64/ARM64; current browser/VNC/runtime/user/port/profile behavior.
...
- Technical facts architecture should verify: Noble package availability and filesystem/runtime path differences for all dependencies; whether any required compatibility correction crosses an approved structural boundary.
